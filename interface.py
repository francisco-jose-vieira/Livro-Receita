...
class InterfaceReceitas:
    ...
    def adicionar_campo_ingrediente(self, container, texto=""):
        ...
        ctk.CTkLabel(frame_linha, text=f"{num}.", font=("Arial Bold", 16), text_color=self.cor_laranja).pack(side="left", padx=(0, 10))
        entrada = ctk.CTkEntry(frame_linha, placeholder_text="Ex: 2 xícaras de açúcar", height=45, fg_color=self.cor_card, border_color="#3D2B1F")
        entrada.insert(0, texto)
        entrada.pack(side="left", fill="x", expand=True)
        ctk.CTkButton(frame_linha, text="🗑", width=40, height=45, fg_color="transparent", text_color="#E74C3C", 
                      command=lambda f=frame_linha, e=entrada: self.remover_linha(f, e, "ing")).pack(side="right", padx=(10, 0))
    ...
    def adicionar_campo_modo(self, container, texto=""):
        ...
        ctk.CTkLabel(frame_linha, text=f"{num}.", font=("Arial Bold", 16), text_color=self.cor_laranja).pack(side="left", padx=(0, 10))
        entrada = ctk.CTkEntry(frame_linha, placeholder_text="Ex: Misture os ovos...", height=45, fg_color=self.cor_card, border_color="#3D2B1F")
        entrada.insert(0, texto)
        entrada.pack(side="left", fill="x", expand=True)
        ctk.CTkButton(frame_linha, text="🗑", width=40, height=45, fg_color="transparent", text_color="#E74C3C",
                      command=lambda f=frame_linha, e=entrada: self.remover_linha(f, e, "modo")).pack(side="right", padx=(10, 0))
    ...
    def criar_tela_principal(self):
        ...
        ctk.CTkButton(header, text="+ Adicionar Receita", fg_color=self.cor_laranja, hover_color=self.cor_laranja_hover,
                      text_color="black", font=("Arial Bold", 15), height=45, corner_radius=10, command=self.tela_adicionar).pack(side="right")

        receitas = self.livro.obter_receitas()
        if not receitas:
            ctk.CTkLabel(main_container, text="Nenhuma receita encontrada no banco de dados.", font=("Arial", 18, "italic"), text_color=self.cor_texto_sec).place(relx=0.5, rely=0.5, anchor="center")
        else:
    ...
    def criar_card(self, container, receita, indice):
        ...
        # EXIBIÇÃO NO CARD: Tempo e Porções lado a lado
        ctk.CTkLabel(card, text=f"⏱ {receita.tempo}  |  🍴 {receita.porcoes}", font=("Arial", 11), text_color=self.cor_laranja).pack(anchor="w", padx=20, pady=(0, 10))
    ...
    def ver_receita(self, indice):
        ...
        ctk.CTkButton(footer, text="← Voltar", fg_color="#38261A", height=55, command=self.criar_tela_principal).pack(side="left", expand=True, fill="x", padx=(0, 15))
        ctk.CTkButton(footer, text="✎ Editar", fg_color=self.cor_laranja, text_color="black", height=55, command=lambda: self.editar_receita(indice)).pack(side="right", expand=True, fill="x", padx=(15, 0))
    ...
    def _exibir_formulario(self, rec_ex=None, ind=None):
        form_wrap = ctk.CTkFrame(self.root, fg_color="transparent")
        form_wrap.pack(pady=20, expand=True, fill="y")
        scr = ctk.CTkScrollableFrame(form_wrap, fg_color="transparent", width=800, height=750)
        ...
        up.pack(fill="x", pady=5); up.pack_propagate(False)
        ...
        ctk.CTkButton(up, text="📷 Alterar Foto", fg_color="transparent", command=lambda: self.selecionar_foto(up)).place(relx=0.5, rely=0.5, anchor="center")

        self.in_nome = ctk.CTkEntry(scr, placeholder_text="Nome da Receita", height=50, fg_color=self.cor_card)
        ...
        # CONFIGURAÇÃO DE TEMPO E PORÇÃO COM PLACEHOLDERS
        for t, var, val, ph in [
            ("TEMPO (apenas número)", "in_tempo", rec_ex.tempo if rec_ex else "", "Ex: 45"),
            ("PORÇÕES (apenas número)", "in_porcao", rec_ex.porcoes if rec_ex else "", "Ex: 4")
        ]:
            ...
            e = ctk.CTkEntry(f, placeholder_text=ph, height=45, fg_color=self.cor_card)
            e.pack(fill="x", pady=5)
            
            # Limpa o texto salvo para o usuário editar só o número
            if val:
                val_limpo = val.replace(" min", "").replace(" porções", "")
                e.insert(0, val_limpo)
            setattr(self, var, e)

        self.c_ing = ctk.CTkFrame(scr, fg_color="transparent"); self.c_ing.pack(fill="x")
        if rec_ex: 
        ...
        ctk.CTkButton(scr, text="+ Ingrediente", fg_color="transparent", border_width=1, text_color=self.cor_laranja, command=lambda: self.adicionar_campo_ingrediente(self.c_ing)).pack(fill="x", pady=10)

        self.c_modo = ctk.CTkFrame(scr, fg_color="transparent"); self.c_modo.pack(fill="x")
        if rec_ex:
            for p in rec_ex.modo_preparo: self.adicionar_campo_modo(self.c_modo, p)
        else: self.adicionar_campo_modo(self.c_modo)
        ctk.CTkButton(scr, text="+ Passo", fg_color="transparent", border_width=1, text_color=self.cor_laranja, command=lambda: self.adicionar_campo_modo(self.c_modo)).pack(fill="x", pady=10)

        ctk.CTkButton(scr, text="Salvar", fg_color=self.cor_laranja, text_color="black", font=("Arial Bold", 18), height=55, command=lambda: self.salvar_receita(ind)).pack(fill="x", pady=30)
        ctk.CTkButton(scr, text="Cancelar", fg_color="transparent", command=self.criar_tela_principal).pack()

    def salvar_receita(self, indice=None):
        # LÓGICA DE TRATAMENTO DOS NÚMEROS E SUFIXOS
        raw_tempo = self.in_tempo.get().strip()
        raw_porcao = self.in_porcao.get().strip()
        
        tempo_final = f"{raw_tempo} min" if raw_tempo else "--"
        porcao_final = f"{raw_porcao} porções" if raw_porcao else "--"

        ingredientes = [e.get() for e in self.entradas_ingredientes if e.get().strip()]
        passos = [e.get() for e in self.entradas_modo if e.get().strip()]
        
        nova_rec = Receita(self.in_nome.get(), ingredientes, passos, tempo_final, porcao_final, self.foto_selecionada)
        
        if indice is not None:
            self.livro.obter_receitas()[indice] = nova_rec
            self.livro.salvar_em_disco()
        else:
            self.livro.adicionar_receita(nova_rec)
